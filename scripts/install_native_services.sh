...
# Default: .deps under the CURRENT directory (documented usage: run from the
# repo root). A script-relative default misresolves when the script runs from
# a copied path (e.g. /tmp), silently installing into the wrong place.
PREFIX="${1:-$(pwd)/.deps}"
case "$PREFIX" in
  /*/*) ;;  # at least two levels deep — sane
  *) echo "refusing to install into ${PREFIX@Q} — pass an explicit prefix"; exit 1 ;;
esac
