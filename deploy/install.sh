...
# signal-cli-rest-api in one container), commissions the instance (config +
# secrets), and starts it under docker compose. Safe to re-run: commissioning
# is skipped if config already exists.
#
#   curl -fsSL https://raw.githubusercontent.com/fughilli/sigbot/master/deploy/install.sh | bash
#
# Env overrides: SIGBOT_HOME (default ~/sigbot), SIGBOT_IMAGE.
set -euo pipefail
...
# One container: sigbot + its bundled signal-cli-rest-api.
services:
  sigbot:
    image: $SIGBOT_IMAGE
    ...
    ports:
      ...
      - "127.0.0.1:8080:8080"   # signal API: account provisioning only
    volumes:
      - ./data:/data
